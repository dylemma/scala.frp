...
class EventSource[A] extends EventStream[A] with Source[Event[A]] with EventSourceImplMixin[A] {
	private val _stopped = new AtomicBoolean(false)
	
	def stopped: Boolean = _stopped.get
	def stop: Unit = if( !_stopped.getAndSet(true) ) produce( Stop ) else println("not stopping")
